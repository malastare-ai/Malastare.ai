...
# A Portugese Banks had conducted a telemarketing campaign for a term-deposit product. Through the campaign, they had 
# collected the data about the prospects' demographics, other financial products they have purchased in the past (loans, deposits etc.) 
# the number of times they were called etc. They also recorded the response data, i.e. whether the person had subscribed to the
# term-deposit product, which is the target variable.
...
## I want to answer the following questions using discriptive data:
...
### 3. Once I've predicted the likelihood of response, how many prospects should one target for telemarketing?
### 4. By how much can one reduce the marketing cost using the model, and how many prospects will one acquire?
...
str(bank_data) #41176 Rows, 21 Columns

# Looking at first few rows
head(sample(bank_data), 10)
...
#--------------------------------------
# Cleaning, Preparation and Exploration
#--------------------------------------
...
# For Outlier Treatment, I'll first identify the outliers and replace outliers with NA values and then impute the values by 
...
# From now, I'll be using complete_data for further analysis
...
# Barplot clearly reflect that age plays a critical role in response and response rate of ages between
# 16 and 20, 60 to 80 are very high.
...
plot_response(complete_data$job, "job") #It shows that the "Retired" and "Student" has large response rates than the rest of the category
# I'll check the age range of "Student" and "Retired" person

qplot(job, age, data = filter(complete_data, job %in% c("student", "retired")), geom = "boxplot")
# Box Plot shows that most of the student are in the range of 17 to 30, while retired are in the range of 55 to 65. 
# So this adds to our age point that people having low and high ages have high response rates.
...
# Plot shows that the Marital Status does not seem to be a good predictor of response
...
# Plot shows that the education does not seem to be a good predictor of response
...
# Plot shows that the Housing Loan does not seem to be a good predictor of response
...
#plot shows that the Personal Loan does not seem to be a good predictor of response
...
# Plot shows that the people who were contacted in the month of Sep, Oct, Dec and March seems to have a very
# high response rate. It may be because these are the months in which people do investment to save on taxes.
...
# Positive Response have average duration of about 498 seconds where as Negative Response has an average of about 214 seconds
...
# Converting this variable to a factor type
...
# Converting this variable to a factor
...
#Plot shows that if the customer is contacted X number of times then there is a high possibility that he will respond
...
# Plot shows that if the previous customer was contacted and he or she responded, then there is a high response rate in next campaign
...
## Like all regression techniques, the logistic regression comes under predictive analysis theory of implementation.
## Logistic regression is used to describe the structure of data and explain the correlation between a dependent binary variable
## and one or more nominal independent variables.
...
### ￼Akaike Information Criteria- AIC is the measure of fitness which can penalize a model for the frequency of
### its model coefficients. Therefore, we always prefer the model with minimum ￼Alkaline Information criteria value for better results

### Null Deviance - represents the outcome predicted by a model w/ the help of the intercept. It all depends if the null
### deviance is less, then the model will be better

### Residual Deviance - describes the response predicted by a model on the addition of independent variables. Same goes
### for residual deviance, the lower the value, the better the results

### Confusion Matrix - is the tabular representaion of actual VS predicted values. It helps in finding the performance of
### a machine learning algorithm, either regression or classification and avoids overfitting
...
print(OUT)
s[9] # 0.08919192
# Taking the cutoff as 0.08919192, at this value we have balanced sensitivity - 67.31%,
# Specificity - 77.74%
# and Accuracy - 76.57%
...
# Making a new data frame which contains the Actual Response, Predicted Response, Predicted Probability of
# Response, Duration of Call in Seconds and cost of each call
...
# Thus the average duration of calls by targeting customers from the first 5 deciles is 275.73 secs with an average cost of Euros:9.31/-
...
# After sucessfully building the model and sorting the data based on decreasing order of probability of 
# response, I came to know that by targeting top 50% (5th decile) prospects, we can reach top 80% of the 
# total responders with minimum cost. 
