...
data.path <- "/cloud/project/MS-DataScience/Quantum_Capital"
...
p <- cor(crude.wide[, 2:5], method = c("pearson")) # is a number between -1 and 1 that indicates how strongly 2 variables are linearly related
getCorTable(p)

k <- cor(crude.wide[, 2:5], method = c("kendall")) # is a statistic used to measure the ordinal association between 2 measured quantities
getCorTable(k)

s <- cor(crude.wide[, 2:5], method = c("spearman")) # is a technique which can be used to summarise the strength and direction (negative or positive) of a relationship between 2 variables. The result will always be between -1 and 1
getCorTable(s)
...
# The first model we are going to look at, focuses solely on predicting the returns
# for WTI as a stand-alone asset as the returns seem to exhibit the characteristics
# of a stationary process at lag = 4.
...
# write.csv(sc.test.data, file = "sc.return.csv") # file saved

# write.csv(sc.transactions, file = "sc.return.csv") # file saved

sc.transactions <- rbind(sc.enter, sc.exit)[ order(Date)][, .(Date, spotPrice, side)]

sc.transactions

# write.csv(sc.transactions, file = "sc.trades.csv") # file saved
...
# write.csv(sc.holdings, file = "sc.positions.csv") # file saved

candlestickHoldings("cl", sc.holdings, sc.test.data, 18.79 )

### BRENT CRUDE 
# The next commodity we are looking at. The 1st thing to do is preview
# the autocorrelations of the TS (time series)
...
# write.csv(cl.transactions, file = "cl.trades.csv") # file saved
...
# write.csv(cl.positions, file = "cl.positions.csv") # file saved

cl.holdings <- getHoldingsFromPositions(cl.positions)


# write.csv(cl.holdings, file = "cl.positions.csv") # file saved

candlestickHoldings("sc", cl.holdings, cl.test.data, cl.return)


### (WTI) Crude Light GARCH
...
# write.csv(rb.test.data, file = "rb.return.csv") # file saved

# write.csv(rb.transactions, file = "rb.return.csv") # file saved
