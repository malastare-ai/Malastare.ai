...
#It takes orders and delivers across the globe and deals with all the major product categories - consumer, corporate 
#& home office. Global Mart needs to finalise the plan for the next 6 months that would help to manage the revenue and inventory
#accordingly. Global Mart needs to find the 2 most profitable and consistent buckets of 21 buckets from 7 different market segments
#and in 3 major categories. After finding the 2 buckets, Global Mart needs to forecast the sales and quantity for next 6 months

#--------------------------------
## Data understanding & Prepartion 
#--------------------------------

# Loading the Data
#----------------

superstore <- read.csv("./Data/Global Superstore.csv", header = TRUE, stringsAsFactors = FALSE)
...
#Creating a new column "Month" to store the difference in number of months from first month
...
#Storing the coefficient of variation of segments and total profit in cov and total_profit columns of sgmt_cov
#dataframerespectively
...
##Above chart clearly shows that the coefficient of variation(COV) of 7th and 4th segments have low values and are
##consistent profitable segments
head(segment7_a)
head(segment4_a)
...
#Making the time series model for "consumer_EU" and "consumer_APAC" segments to predict the sales and quantity for
#the next 6 months
...
# Let's create the model on first 42 rows and remaining 6 rows will be used for evaluation
#----------------------------------------------------------------------------------------
...
##Model successfully made using "classical decomposition" and "auto ARIMA", also Sales and Quantity are predicted for
##next 6 months
